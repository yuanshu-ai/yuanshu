...
export function AdminApp() {
  ...
  return <main className="admin-shell">
    ...
    <aside className="admin-nav" aria-label="Server 管理导航">
      <a className="admin-brand" href="/"><BrandMark /><span><strong>Yuanshu</strong><small>{t("admin.title")}</small></span></a>
...
function Nodes({ items, invitations, details, onAdd, onCancelInvitation, onReissueInvitation, onInspect, onRevoke }: { items: AdminNode[]; invitations: AdminNodeInvitation[]; details: Record<string, AdminNodeDetail>; onAdd: () => void; onCancelInvitation: (item: AdminNodeInvitation) => void; onReissueInvitation: (item: AdminNodeInvitation) => void; onInspect: (id: string) => void; onRevoke: (item: AdminNode) => void }) { const { t } = useI18n(); return <div className="admin-stack"><section className="admin-block"><div className="block-heading"><div><h2>Node</h2><p>Server 只创建一次性邀请，Node 始终主动建立出站连接。</p></div><button className="secondary-button" onClick={onAdd}>{t("admin.addNode")}</button></div><AdminList empty="还没有已注册 Node">{items.map((item) => <div className="node-detail-group" key={item.id}><article className="admin-row"><div><span className={`semantic-dot ${item.online ? "ok" : "quiet"}`} /><strong>{item.name}</strong><small>{item.os} / {item.version}</small></div><div className="row-meta"><span>{item.online ? machineStatus("online")!.title : machineStatus("offline")!.title}</span><span>最近连接 {formatDate(item.lastSeenAt)}</span></div><div className="row-actions"><button className="secondary-button" onClick={() => void onInspect(item.id)}>详情</button><button className="danger-button" disabled={item.status !== "active"} onClick={() => onRevoke(item)}>撤销</button></div></article>{details[item.id] && <NodeRuntimeDetail value={details[item.id]} />}</div>)}</AdminList></section><section className="admin-block"><h2>{t("admin.invitation.title")}</h2><AdminList empty="没有邀请记录">{invitations.map((item) => <article className="admin-row compact" key={item.invitationId}><div><strong>{item.displayName}</strong><small>{shortID(item.invitationId)}</small></div><div className="row-meta"><span>{item.status}</span><span>{formatDate(item.expiresAt)} 过期</span></div><div className="row-actions">{item.status === "pending" ? <button className="secondary-button" onClick={() => onCancelInvitation(item)}>{t("admin.invitation.cancel")}</button> : <button className="secondary-button" onClick={() => onReissueInvitation(item)}>{t("admin.invitation.reissue")}</button>}</div></article>)}</AdminList></section></div>; }
