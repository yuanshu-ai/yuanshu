...
import { AdminClient, type AdminAccessRequest, type AdminAudit, type AdminConfig, type AdminControlClient, type AdminLease, type AdminNode, type AdminOverview } from "./admin-client";
import "./admin.css";

type Section = "overview" | "nodes" | "clients" | "access" | "security";
type AdminData = { overview?: AdminOverview; nodes: AdminNode[]; clients: AdminControlClient[]; requests: AdminAccessRequest[]; leases: AdminLease[]; audit: AdminAudit[]; config?: AdminConfig; diagnostics?: Record<string, unknown> };
type Confirmation = { title: string; detail: string; confirmLabel: string; requiredText?: string; run: () => Promise<void> };

const emptyData: AdminData = { nodes: [], clients: [], requests: [], leases: [], audit: [] };
...
export function AdminApp() {
  ...
  const refresh = useCallback(async (active: AdminClient, quiet = false) => {
    ...
    try {
      const [overview, nodes, clients, requests, leases, audit, config, diagnostics] = await Promise.all([
      ...
      setData({ overview, nodes: nodes.nodes, clients: clients.controlClients, requests: requests.requests, leases: leases.leases, audit: audit.audit, config, diagnostics });
      setState("ready");
      if (!quiet) setMessage("状态已更新");
    } catch (error) {
      if (!alive.current) return;
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-main">
      ...
      {section === "nodes" && <Nodes items={data.nodes} onRevoke={revokeNode} />}
...
function Security({ data, onDownload }: { data: AdminData; onDownload: () => void }) { const config = data.config; return <div className="admin-stack"><section className="admin-split"><article className="admin-block"><div className="block-heading"><h2>脱敏 Server 配置</h2><span>只读</span></div><dl><DataRow label="Listen" value={config?.listen || "未报告"} /><DataRow label="Public URL" value={config?.publicUrl || "loopback"} /><DataRow label="Web" value={config?.webEnabled ? "启用" : "关闭"} /><DataRow label="Admin" value={config?.adminEnabled ? "启用" : "关闭"} /><DataRow label="TLS" value={config?.tls.configured ? "已配置" : "未配置"} /></dl></article><article className="admin-block"><div className="block-heading"><div><h2>诊断摘要</h2><p>不包含凭据、任务正文或绝对路径。</p></div><button className="secondary-button" onClick={onDownload}>下载 JSON</button></div><dl><DataRow label="配置 revision" value={shortID(config?.configRevision)} /><DataRow label="允许 Origin" value={String(config?.allowedControlOrigins.length ?? 0)} /><DataRow label="TLS SAN" value={config?.tls.san?.join(", ") || "无"} /><DataRow label="证书到期" value={formatDate(config?.tls.notAfter)} /></dl></article></section><RecentAudit items={data.audit} /></div>; }
