...
export function NewTaskFlow({ session, connectionState, nodes, workspaces, initialTarget, onClose, onConfirmed }: {
  ...
  const submit = async (event?: FormEvent) => {
    event?.preventDefault();
    if (!node || !workspace || !nodeAvailable(node) || !input.trim() || busy || connectionState !== "connected") return;
    setBusy(true);
    setMessage("");
    try {
  ...
  if (confirmDiscard) {
    return <Dialog title="放弃未发送的任务？" destructive onClose={() => setConfirmDiscard(false)} actions={<>
      <button className="button secondary" type="button" onClick={() => setConfirmDiscard(false)}>继续编辑</button>
      <button className="button danger solid" type="button" onClick={onClose}>放弃草稿</button>
    </>}><p>任务要求尚未发送。离开后这份草稿不会保存在浏览器中。</p></Dialog>;
  }

  return <Dialog className="new-task-dialog" title="开始新任务" onClose={requestClose} actions={<FlowActions step={step} busy={busy} canContinue={step === "target" ? Boolean(workspace && nodeAvailable(node)) : step === "compose" ? Boolean(input.trim()) : connectionState === "connected"} onBack={() => setStep(step === "review" ? "compose" : "target")} onNext={() => setStep(step === "target" ? "compose" : "review")} onClose={requestClose} onSubmit={() => void submit()} />}>
    <div className="new-task-flow">
      ...
      {step === "compose" && <form className="new-task-compose" onSubmit={(event) => { event.preventDefault(); if (input.trim()) setStep("review"); }}><TargetSummary node={node} workspace={workspace} compact /><label htmlFor="new-task-input">你希望 Codex 完成什么？</label><textarea id="new-task-input" autoFocus value={input} onChange={(event) => setInput(event.target.value)} placeholder="描述目标、约束和完成标准" rows={8} disabled={busy} /></form>}
...
function TargetSummary({ node, workspace, compact = false }: { node?: NodeProjection; workspace?: WorkspaceProjection; compact?: boolean }) {
  return <section className={`target-summary ${compact ? "compact" : ""}`} aria-label="执行目标"><div><span>设备</span><b>{node?.name ?? "未选择"}</b><StatusPill tone={nodeAvailable(node) ? "accent" : "warning"}>{nodeAvailable(node) ? "在线" : "不可用"}</StatusPill></div><div><span>工作区</span><b>{workspace?.name ?? "未选择"}</b></div><div><span>权限</span><b>{permissionLabel(workspace?.permissionProfile)}</b></div><div><span>网络</span><b>{networkLabel(workspace?.allowNetwork)}</b></div></section>;
}
...
function nodeAvailable(node?: NodeProjection): boolean {
  return Boolean(node?.online && !["unavailable", "not_available", "offline"].includes(node.runtimeStatus ?? ""));
}
...
function validInitialTarget(target: NewTaskTarget | undefined, nodes: NodeProjection[], workspaces: WorkspaceProjection[]): NewTaskTarget | undefined {
  ...
  return nodeAvailable(node) && workspace ? target : undefined;
}
