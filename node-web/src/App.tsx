...
function OverviewPanel({ value, run, updateConfig, busy }: { value: Overview; run: (command: string, fields?: Record<string, unknown>) => Promise<unknown>; updateConfig: (changes: Record<string, unknown>) => Promise<void>; busy: boolean }) {
  ...
  return <div className="stack"><section className="hero-panel"><div><StatusMark state={status.state} large /><h2>{stateLabel(status.state)}</h2><p>{next.detail}</p></div><button className="primary" disabled={busy} onClick={() => void run(next.command)}>{next.label}</button></section><section className="status-grid"><StatusRow label="Relay" value={status.remoteControl} /><StatusRow label="Codex" value={status.codex} /><StatusRow label="身份" value={status.identity} /><StatusRow label="密钥存储" value={status.identityStorage === "local_file" ? "Node 私有目录" : status.identityStorage || "未确认"} /><StatusRow label="恢复" value={status.recovery} /><StatusRow label="工作区" value={String(status.workspaces)} /><StatusRow label="登录启动" value={status.autostart} /></section><section className="section-panel"><h2>Node 名称</h2><p className="helper">名称会显示在你自己的控制端中，不包含设备路径或身份凭据。</p><form className="settings-form" onSubmit={(event) => { event.preventDefault(); void updateConfig({ hostName: name }); }}><label><span>显示名称</span><input value={name} maxLength={128} required onChange={(event) => setName(event.target.value)} /></label><div className="actions"><button className="secondary" type="button" disabled={busy || status.autostart === "unavailable"} onClick={() => void run("autostart_set", { enabled: status.autostart !== "enabled" })}>{status.autostart === "enabled" ? "关闭登录启动" : "启用登录启动"}</button><button className="primary" disabled={busy || !name.trim()} type="submit">保存名称</button></div></form></section></div>;
}
...
function Notice({ children, tone = "info" }: { children: ReactNode; tone?: "info" | "danger" }) { return <div className={`notice ${tone}`} role="status">{children}</div>; }
