...
func Open(ctx context.Context, path string, options Options) (*Store, error) {
	...
	if err != nil || !parent.IsDir() {
		return nil, errors.New("server store parent directory is unavailable")
	}
	if info, err := os.Lstat(path); err == nil {
		...
		return nil, errors.New("server store path is unavailable")
	}
	file, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE, 0o600)
	if err != nil {
		return nil, errors.New("server store file is unavailable")
	}
	if err := file.Chmod(0o600); err != nil {
		_ = file.Close()
		return nil, errors.New("server store permissions could not be applied")
	}
	if err := file.Close(); err != nil {
		return nil, errors.New("server store file could not be prepared")
	}
	...
	local := &Store{db: db, clock: options.Clock}
	if local.clock == nil {
		local.clock = time.Now
	}
...
func Inspect(ctx context.Context, path string) (Inspection, error) {
	...
	if result.SchemaVersion > CurrentSchemaVersion {
		return Inspection{}, ErrFutureSchema
	}
	if result.SchemaVersion != CurrentSchemaVersion {
		return Inspection{}, ErrCorrupt
	}
...
func (s *Store) initialize(ctx context.Context) error {
	...
	for _, statement := range []string{"PRAGMA foreign_keys = ON", "PRAGMA synchronous = FULL", "PRAGMA busy_timeout = 5000"} {
	...
	if err := s.db.QueryRowContext(ctx, "PRAGMA journal_mode = WAL").Scan(&mode); err != nil || mode != "wal" {
		return errors.New("server store WAL mode is unavailable")
	}
...
func (s *Store) Close() error {
	...
	if err := s.db.Close(); err != nil {
		return errors.New("server store close failed")
	}
	return nil
}
...
func internal(operation string) error { return fmt.Errorf("server store %s failed", operation) }
