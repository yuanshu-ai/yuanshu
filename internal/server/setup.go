...
<form id="form"><div class="card"><label>部署模式<select name="mode"><option value="local">本机（HTTP/WS）</option><option value="lan-managed">局域网托管 CA（推荐）</option><option value="public-ip-acme">公网 IP ACME</option><option value="external">外部证书/反向代理</option></select></label><label>数据目录<input name="dataDir" required></label><label>监听地址<input name="listen" list="listen-addresses" required><datalist id="listen-addresses"></datalist></label><label>浏览器公开 URL<input name="publicURL" placeholder="https://192.168.1.20:9527"></label></div>
...
func localInterfaceAddresses() []string {
	...
	seen := map[string]bool{DefaultListenAddress: true, DefaultIPv6Listen: true}
	result := []string{DefaultListenAddress, DefaultIPv6Listen}
	for _, item := range interfaces {
		...
		for _, address := range addresses {
			...
			value := net.JoinHostPort(host.String(), DefaultPort)
			if !seen[value] {
				seen[value] = true
